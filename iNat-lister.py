...
#Todo -- exclude casual observations, wah. -- there is currently causiging a slight mismatch between what iNat and my count show, since I think my 
# queries here exclue casual, but the iNat links include it
#I think makes more sense to include everythings
...
#response = get_observation_taxonomy(project_id= project, verifiable=True, ident_user_id=318825) # all bees I identified for a given project, need to find all bees project
...
for entry in data:
    ...
    #another dumb hack because "State of matter life" is different than everything else for some dumb reason
    if entry["id"] == 48460:
        entry["name"] = "State of matter life"
        entry["rank"] = "Life"
        entry["rank_level"] = 1
        entry["parent_id"] = 0
...
for taxa in all_taxa:
    ...
    if taxa[3] != "species" and taxa[3] != "subspecies":
        ...
        link = "https://www.inaturalist.org/observations?project_id=" + str(project) + "&place_id=any&verifiable=any&captive=any&taxon_id=" + str(taxa[0]) + "&lrank=" + taxa[3]
        
        
        #do observations with children (total observations)
        link2 = "https://www.inaturalist.org/observations?project_id=" + str(project) + "&place_id=any&verifiable=any&captive=any&taxon_id=" + str(taxa[0]) 
        ...
        link = "https://www.inaturalist.org/observations?project_id=" + str(project) + "&place_id=any&verifiable=any&captive=any&taxon_id=" + str(taxa[0]) 
...
f = open("nesting_page_full_info.html", 'w')
f.write(information_string)
f.close()

f = open("nesting_page_formatted.html", 'w')
